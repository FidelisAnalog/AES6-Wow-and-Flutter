...
def analyzeFull(data, sampleRate=None, inputType='audio',
    ...
    if inputType == 'device':
        return _analyze_device(data, rpm=rpm)
    else:
        if sampleRate is None:
            raise ValueError("sampleRate is required for audio input")
        return _analyze_audio(data, sampleRate)


def _analyze_audio(pcm_data, sample_rate):
    """Full audio pipeline."""
    fs = sample_rate
    ...
    _status("Cleaning frequency data...")
    ...
    _status("Computing AM/FM coupling...")
    ...
    am_envelope = np.abs(hilbert(sig_filtered))
    ...
    am_t_raw = np.arange(len(am_pct)) / fs + (trim_am / fs)
...
def _analyze_device(text_data, rpm=None):
    """Device input pipeline — enters at deviation stage."""
    _status("Detecting device format...")
