...
def find_zero_crossings(sig, fs, hysteresis_frac=0.05, sinc_taps=32):
    """
    Find positive-going zero crossings with hysteresis and sinc-interpolated
    root finding.
    ...
    2. Hysteresis: signal must drop below -threshold to arm, then trigger
       ...
       false crossings.
    3. For each crossing, use windowed-sinc interpolation to reconstruct
       the continuous signal between the bracketing samples, then use
       Brent's method to find the exact zero location.

    This correctly applies the sampling theorem — the zero crossing is
    located on the true bandlimited signal, not a linear approximation.
    ...
    # Hysteresis validation: each crossing must have an arm point
    # between it and the previous crossing
    valid = np.zeros(len(zc_indices), dtype=bool)
    prev_zc = 0
    arm_pos = 0
    for k in range(len(zc_indices)):
        zc = zc_indices[k]
        while arm_pos < len(arm_indices) and arm_indices[arm_pos] < prev_zc:
            arm_pos += 1
        if arm_pos < len(arm_indices) and arm_indices[arm_pos] < zc:
            valid[k] = True
        prev_zc = zc

    crossing_indices = zc_indices[valid]

    # Discard crossings too close to signal boundaries where the sinc
    # kernel would be truncated and give incorrect interpolation.
    margin = sinc_taps
    crossing_indices = crossing_indices[
        (crossing_indices >= margin) & (crossing_indices < len(sig) - margin - 1)
    ]

    # For each validated crossing, find the exact zero using sinc interpolation
    # and Brent's root-finding method.
    # The crossing is between sample crossing_indices[k] and crossing_indices[k]+1.
    times = np.empty(len(crossing_indices), dtype=np.float64)

    for i, ci in enumerate(crossing_indices):
        # Define function: sinc-interpolated signal value at fractional position
        f = lambda frac: sinc_interp_at(sig, ci + frac, n_taps=sinc_taps)

        # Brent's method finds the root in [0, 1] to machine precision
        frac = brentq(f, 0.0, 1.0, xtol=1e-14, rtol=1e-14)
        times[i] = (ci + frac) / fs
...
def interpolate_to_uniform(times, freqs, output_rate=None, sinc_taps=16):
    """
    Resample non-uniform frequency samples to a uniform time grid using
    windowed-sinc interpolation (bandlimited reconstruction).

    The per-cycle frequency samples have negligible timing jitter (<0.1%
    of the sample spacing), so we treat them as uniformly sampled and
    apply standard sinc interpolation.  This avoids the position-dependent
    smoothing artifact of linear interpolation, where the uniform output
    grid drifts in and out of phase with the non-uniform input grid over
    the file duration, creating a U-shaped noise envelope.
    ...
    # Mean input spacing — basis for treating input as uniform
    dt_raw = (times[-1] - times[0]) / (len(times) - 1)

    # Start/end the uniform grid sinc_taps samples in from each edge
    # so the sinc kernel always has full symmetric support.  This
    # eliminates the one-sided interpolation artifact at boundaries.
    t_start = times[0] + sinc_taps * dt_raw
    t_end = times[-1] - sinc_taps * dt_raw
    t_uniform = np.arange(t_start, t_end, 1.0 / output_rate)

    # Convert each output time to a fractional sample position in the
    # input array, then reconstruct via windowed sinc.
    positions = (t_uniform - times[0]) / dt_raw
    f_uniform = np.empty(len(t_uniform), dtype=np.float64)
    for j in range(len(t_uniform)):
        f_uniform[j] = sinc_interp_at(freqs, positions[j], n_taps=sinc_taps)
